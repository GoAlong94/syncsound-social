...
export const ENGINE_VERSION = "v6.4-ContinuousNTP";
...
export const useSyncEngine = ({
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'pong' }, ({ payload }) => {
      if (!isHost && payload.target === userId && !isNtpFrozenRef.current) {
        ...
        // After 45 fast pings, freeze the fast phase but keep a slow background refresh.
        // Prevents clock drift accumulation over long sessions without churning the offset.
        if (pingCountRef.current > 45 && !isNtpFrozenRef.current) {
            isNtpFrozenRef.current = true;
            logEvent('NTP_FROZEN_LOCKED', { finalOffset: metrics.offset, finalJitter: metrics.jitter });

            // Slow refresh: 3 pings every 60s — keeps offset honest without disrupting sync
            ntpSlowIntervalRef.current = setInterval(() => {
                isNtpFrozenRef.current = false;
                pingCountRef.current = 43; // allow 3 more pongs through before re-freezing
                logEvent('NTP_SLOW_REFRESH', { prevOffset: clockOffsetRef.current });
            }, 60_000);
        }
    ...
    return () => { if (catchupTimeout.current) clearTimeout(catchupTimeout.current); if (ntpSlowIntervalRef.current) clearInterval(ntpSlowIntervalRef.current); channel.unsubscribe(); };
  ...
  useEffect(() => {
    ...
    }, 100); // v6.4: 200ms → 100ms — tighter detection without over-sampling
  ...
  const manualResyncFunc = useCallback(() => {
    ...
    if (ntpSlowIntervalRef.current) { clearInterval(ntpSlowIntervalRef.current); ntpSlowIntervalRef.current = null; }
